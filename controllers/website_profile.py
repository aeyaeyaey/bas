...
class CustomWebsiteProfile(WebsiteProfile):
    ...
    def _profile_edition_preprocess_values(self, user, **kwargs):
        values = {
            ...
            'nationality_id': int(kwargs.get('nationality_id')) if kwargs.get('nationality_id') else False,
    ...
    def save_edited_profile(self, **kwargs):
        ...
        if 'nationality_id' in values:
            whitelisted_values['nationality_id'] = values['nationality_id']
        print(values)
    ...
    def get_ilce(self, il_id):
        if il_id.isdigit():  # il_id'nin sayısal olup olmadığını kontrol edin
            ilceler = request.env['bas.ilce'].sudo().search([('il_id', '=', il_id)])
            return [{'id': ilce.id, 'name': ilce.name} for ilce in ilceler]
        else:
            return []  # Geçersiz bir il_id ise boş bir liste döndürün

    @http.route('/get_lise', type='json', auth='user', methods=['POST'], website=True)
    def get_lise(self, ilce_id):
        if ilce_id.isdigit():  # ilce_id'nin sayısal olup olmadığını kontrol edin
            liseler = request.env['bas.lise'].sudo().search([('ilce_id', '=', ilce_id)])
            return [{'id': lise.id, 'name': lise.name} for lise in liseler]
        else:
            return []  # Geçersiz bir ilce_id ise boş bir liste döndürün

    @http.route('/get_fak', type='json', auth='user', methods=['POST'], website=True)
    def get_fak(self, uni_id):
        if uni_id.isdigit():  # uni_id'nin sayısal olup olmadığını kontrol edin
            fakulteler = request.env['bas.fak'].sudo().search([('uni_id', '=', uni_id)])
            return [{'id': fak.id, 'name': fak.name} for fak in fakulteler]
        else:
            return []  # Geçersiz bir uni_id ise boş bir liste döndürün

    @http.route('/get_bolum', type='json', auth='user', methods=['POST'], website=True)
    def get_bolum(self, fak_id):
        if fak_id.isdigit():  # fak_id'nin sayısal olup olmadığını kontrol edin
            bolumler = request.env['bas.bolum'].sudo().search([('fak_id', '=', fak_id)])
            return [{'id': bolum.id, 'name': bolum.name} for bolum in bolumler]
        else:
            return []  # Geçersiz bir fak_id ise boş bir liste döndürün